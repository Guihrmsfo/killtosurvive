<%-- 
    Document   : equipments
    Created on : 28/10/2012, 21:57:28
    Author     : Guilherme
--%>
...
<c:choose>
    ...
    <c:when test="${param.equipmentName!= null}">
        <%ManipularDados.alterarEquipamentoDeUso(session.getAttribute("usuario"), request.getParameter("equipmentName"));%>
    </c:when>
                                    ...
                                    <center>
                                        <table width="550">
                                            <tr>
                                                <td align="center">
                                                    <p class="title">
                                                        <fmt:message key="equipmentspage.text.equipments"/>
                                                    </p>

                                                    <c:if test="${fn:length(sessionScope.usuario.personagem.equipamentos)==0}">
                                                        <fmt:message key="equipmentspage.text.noequipment"/>
                                                    </c:if>

                                                    <c:forEach items="${sessionScope.usuario.personagem.equipamentos}" begin="0"  step="1" var="equipamento">
                                                        <h:equipments language="${language}" name="${equipamento.nome}" description="${equipamento.descricao}" defense="${equipamento.defesa}" actualEquipment="${sessionScope.usuario.personagem.equipamentoAtual.nome}"/>
                                                    </c:forEach>
                                                </td>
                                            </tr>
                                        </table>
